...
const display = (data)=> {
  ...
  const display =  data.map((v,index)=>{
     ...
     <button type="button" class="filter-btn" data-id="addtocart"   >Add to Cart</button>
   </article>`
 ...
 Dscprice = ()=>{
  ...
  const addMenu = (myindex)=>{
    ...
    const selectedMenu= menu1[myindex];
    const findMenu = carts.find((v)=>{

        return v.item.id == selectedMenu.id
    })

    if(!findMenu)
    {

        carts.push({item: selectedMenu,count:1});
    }
    else
    {
